...
8. **Zero dependencies** — No external crates.

## 2. Project Structure

**File layout** (unchanged by refactoring):

```
analysis-project/
  Cargo.toml            # crate "analysis", edition 2024, zero dependencies
  example.log           # sample log data for manual testing
  src/
    lib.rs              # library crate: public API, filtering, iteration
    parse.rs            # parser combinators + log data model
    main.rs             # CLI binary ("cli"): reads file, prints parsed logs
  docs/
...
| `parse.rs` | `Parser` / `Parsable` traits, combinators (`Tag`, `Alt`, `Map`, `Delimited`, ...), log types (`LogLine`, `LogKind`, etc.) |
...
- Hand-rolled parser combinators (no external deps, nom-like API)
- Parsers are zero-sized stateless structs — cheap to construct
- Combinator arities via tuple impls (`all2`..`all4`, `alt2`..`alt8`) since Rust lacks variadic generics
- `do_unquote()` intentionally returns `String` (deserialized value, not a slice)
